...
def get_txt_location():
    txtlocurl = "https://www.dropbox.com/s/kbghwiqyep9n8if/pokeloc.txt?dl=1"
    data = urllib2.urlopen(txtlocurl) # it's a file like object and works just like a file
    locationfromtxt = [x for x in data][-1]
    return locationfromtxt
...
def main():
    ...
    if not (FLOAT_LAT and FLOAT_LONG):
      ...
      retrying_set_location(get_txt_location()) #gilbert  args.location
...
def next_loc():
    ...
    #     print('[+] Saved next location as %s,%s' % (lat, lon))
        #NEXT_LAT = float(lat)
        #NEXT_LONG = float(lon)
    NEXT_COOR = get_txt_location()
    print('[+] Saved next location as %s' % (NEXT_COOR))
    return 'ok'
